...
, zlib, libusb1, udev, glibc, ncurses5, python3, python310, python311, python312
, libxml2_13 }:
...
let
  ...
  toolSpecToDerivation = toolSpec:
    let
      ...
      targetVersionSpec =
        targetVersionSpecs.${targetPlatform} or targetVersionSpecs.any;
      platformOverrides =
        builtins.filter (o: lib.elem targetPlatform o.platforms)
        (toolSpec.platform_overrides or [ ]);
      mergedToolSpec =
        lib.foldl' lib.recursiveUpdate toolSpec platformOverrides;
    in mkToolDerivation {
      ...
      exportPaths = builtins.filter (path: path != [ "" ])
        (mergedToolSpec.export_paths or [ ]);
    };

  mkToolDerivation = { pname, version, description, homepage, license, url
    , sha256, runtimeDeps, exportVars, stripContainerDirs, exportPaths }:

    let
      binPaths = map
        (path: lib.foldl' (a: b: if a == "" then b else "${a}/${b}") "" path)
        exportPaths;
      exportVarsWrapperArgsList = lib.attrsets.mapAttrsToList
        (name: value: ''--set "${name}" "${value}"'') exportVars;
    in stdenv.mkDerivation (finalAttrs: {
      ...
      phases = [ "unpackPhase" "installPhase" ]
        ++ lib.optionals stdenv.isLinux [ "fixupPhase" ];
          ...
          if [ -d "$out/$bindir" ]; then
            for file in $out/$bindir/*; do
              if [ -f "$file" ] && [ -x "$file" ]; then
                ...
                makeWrapper "$file" "$wrapper_file" ${
                  lib.strings.concatStringsSep " " exportVarsWrapperArgsList
                }
              fi
            done
          fi
      ...
      passthru = let inherit (finalAttrs.finalPackage) outPath;
      in {
        exportVars = lib.mapAttrs (_: v:
          lib.replaceStrings [ "\${TOOL_PATH}" "$TOOL_PATH" ] [
            outPath
            outPath
          ] v) exportVars;
      };
...
in builtins.listToAttrs (builtins.map (toolSpec:
  lib.attrsets.nameValuePair toolSpec.name (toolSpecToDerivation toolSpec))
  toolSpecList)
