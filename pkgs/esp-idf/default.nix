{ owner ? "espressif", repo ? "esp-idf", rev ? "v5.5.2"
...
, toolsToInclude ? [
  ...
  "openocd-esp32"
  "esp-rom-elfs"
], extraPythonPackages ? (pythonPackages: [ ]), stdenv, lib, fetchFromGitHub
...
in esp-idf // {
  examples = lib.genAttrs targets (target:
    callPackage ./examples.nix { inherit esp-idf buildExample target; });
}
